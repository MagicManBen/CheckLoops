...
async function fixAchievements(email = 'benhowardmagic@hotmail.com') {
  ...
  try {
    ...
    if (!authUser) {
      console.error('❌ Could not find user in auth.users');
      return;
    }
    ...
    const { data: profileData, error: authError } = await supabase
      .from('profiles')
      .select('user_id, site_id, kiosk_user_id')
      .eq('user_id', userId)
      .maybeSingle();

    if (authError || !profileData) {
      console.error('❌ Could not find user profile:', authError);
      return;
    }

    // userId already defined above
    const siteId = profileData.site_id || 2;
    let kioskUserId = profileData.kiosk_user_id;
