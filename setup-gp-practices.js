...
async function fetchFromNHSOrd(offset = 0, limit = 1000) {
  const urls = [
    `https://directory.spineservices.nhs.uk/ORD/2-0-0/organisations?Status=Active&Limit=${limit}&Offset=${offset}&_format=json`,
    `https://directory.spineservices.nhs.uk/ORD/2-0-0/organisations?Status=Active&NonPrimaryRoleId=RO76&Limit=${limit}&Offset=${offset}&_format=json`,
    `https://directory.spineservices.nhs.uk/ORD/2-0-0/organisations?Status=Active&PrimaryRoleId=RO177&Limit=${limit}&Offset=${offset}&_format=json`,
  ];
...
async function loadGPPractices() {
  ...
  while (hasMore) {
    ...
    const practices = data.Organisations.map(org => ({
      ...
      primary_role: org.Roles?.Role?.[0]?.id || '',
      raw_data: org
