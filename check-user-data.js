...
async function checkUserData() {
  try {
    console.log('🔍 Checking user data for benhowardmagic@hotmail.com');
    ...
    if (profileError) {
      ...
      if (profiles[0].site_id) {
        ...
        console.log('❌ NO SITE_ID in profile!');
      }
    }
    ...
    if (inviteError) {
      ...
      console.log('ℹ️  No site invites found');
    } else {
      console.log('✅ Site invites found:', invites);
    }
    ...
    if (profiles.length === 0) {
      ...
      console.log('❌ ISSUE: User profile exists but has no site_id');
      console.log('   Need to assign user to a site');
    } else if (!profiles[0].full_name) {
      console.log('⚠️  WARNING: User profile has no full_name');
      console.log('   Will show email-derived name');
    } else {
      console.log('✅ Profile data looks complete');
    }
